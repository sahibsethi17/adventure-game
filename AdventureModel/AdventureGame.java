...
public class AdventureGame implements Serializable {
    ...
    public boolean movePlayer(String direction) {
        ...
        for (Passage entry : possibilities) {
            ...
            } else if (chosen == null && entry.getIsLocked()) {
                if (this.player.getInventory().containsAll(entry.getSecretKeys())) {
                    chosen = entry; //we can make it through, given our stuff
                    break;
                }
            }
        ...
        AchievementList.getInstance().checkCategory("secret", this);
        ...
        if (motionTable.optionExists(inputArray[0])) {
            if (!movePlayer(inputArray[0])) {
                if (this.player.getCurrentRoom().getMotionTable().getDirection().get(0).getDestinationRoom() == 0) {
                    AchievementList.getInstance().checkCategory("completion", this);
                    AchievementList.updateScoreString();
                    AchievementList.saveAchievements(this);
                    return "GAME OVER";
                }else return "FORCED";
    ...
    /**
     * @return a string containing all visited room names separated by new lines
     */
    public String getVisitedRoomsString() {
        String s = "";
